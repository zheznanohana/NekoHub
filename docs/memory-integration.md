...
**第二层：向量，本机跑，默认开启。** `MEMORY_EMBED_BACKEND=local` 用 fastembed
（ONNX，不拖 torch）在本机推理：不需要 key，查询时不联网，数据不出机器。
也支持任意 OpenAI 兼容 `/embeddings` 端点（把 backend 设成别的值 + 填 BASE_URL）。
向量与 BM25 用 RRF 融合，按 batch 增量生成，存在 `memory_vectors`，内容变了自动失效。
未配置时只跑 BM25；模型加载或推理失败自动降级为 BM25，不会让查询失败。
`GET /status` 的 `retrieval` 字段（`lexical` / `hybrid`）告诉前端实际跑的是哪一层。

**选型是实测出来的**（1124 条真实数据，400 条抽样，查 6 个词看相关项是否进前 5）：

| 模型 | 命中 | 400 条耗时 | 体积 |
| --- | --- | --- | --- |
| **BAAI/bge-small-zh-v1.5** | **5/6** | **37s** | **90MB** |
| paraphrase-multilingual-MiniLM-L12-v2 | 3/6 | 20s | 220MB |
| jinaai/jina-embeddings-v2-base-zh | 3/6 | 346s | 640MB |

所以默认用 bge-small-zh-v1.5。全库 1124 条向量化耗时 74 秒。

开启后实测：`群组` 0→9、`下载` 0→24、`下载进度` 0→命中 EhViewer apk 下载、
`月结单` 0→命中 ZA Bank 电子月结单、`安装包` 0→命中安装相关。
**仍然不对的：`电报` 的首位命中是 HSBC 电子结单而不是 Telegram。**
bge 是中文模型，`电报`↔`Telegram` 这种跨语言别名它对不上；三个模型都没对上。
搜 `Telegram` 本身是准的。这一条别当成已经解决。

## 语义相似连线

有了向量之后，`memory.network` 除了压缩/实体/证据三种边，还会算一类 `similar` 边：
对当前视图内的节点两两算余弦，取每个节点最相似的 3 个且超过 0.62 才连。
实测让图从 109 条边涨到 288 条（109 compresses + 179 similar）。

**这类边是算出来的，不是断言的关系**，所以单独一种 kind、单独一种线型（点划线），
scope 文案和提示词都写明了这一点——Agent 不能把「像」当成「有关系」来引用。
